...
${u.blue("Please consider disable `cacheHTML` and `cachePic`, or set `maxSize` and `maxPicSize` option.\n")}`)})},d=(e,a)=>a.startsWith("/")?a.replace(/^\//,e):a,N=(e,a)=>(a.icons&&(a.icons=a.icons.map(n=>({...n,src:d(e,n.src)}))),a.shortcuts&&(a.shortcuts=a.shortcuts.map(n=>{var t;return{...n,icons:((t=n.icons)==null?void 0:t.map(o=>({...o,src:d(e,o.src)})))||[],url:d(e,n.url)}})),a.screenshots&&(a.screenshots=a.screenshots.map(n=>({...n,src:d(e,n.src)}))),a),D=(e,a)=>{a.favicon&&(a.favicon=d(e,a.favicon)),a.apple&&(a.apple.icon&&(a.apple.icon=d(e,a.apple.icon)),a.apple.maskIcon&&(a.apple.maskIcon=d(e,a.apple.maskIcon))),a.msTile&&a.msTile.image&&(a.msTile.image=d(e,a.msTile.image)),a.manifest&&(a.manifest=N(e,a.manifest))},B=(e,a="/",n=[])=>{var t;const o=[],l=[];n.forEach(c=>{c[0]==="meta"?o.push(c[1].name):c[0]==="link"&&l.push(c[1].rel)});let i="";const s=(c,p,g={})=>{l.includes(c)||n.push(["link",{rel:c,href:p,...g}])},r=(c,p,g={})=>{o.includes(c)||n.push(["meta",{name:c,content:p,...g}])};if(e.favicon&&s("icon",e.favicon),(t=e.manifest)!=null&&t.icons){const{icons:c}=e.manifest;c.length>0&&(i=c[0].src,e.manifest.icons.map(p=>{p.type?s("icon",p.src,{type:p.type,sizes:p.sizes}):s("icon",p.src,{sizes:p.sizes})}))}return s("manifest",`${a}manifest.webmanifest`,{crossorigin:"use-credentials"}),r("theme-color",e.themeColor||"#46bd87"),typeof e.apple=="object"&&(e.apple.icon||i)?(s("apple-touch-icon",e.apple.icon||i),r("apple-mobile-web-app-capable","yes"),r("apple-mobile-web-app-status-bar-style",e.apple.statusBarColor||"black"),e.apple.maskIcon&&s("mask-icon",e.apple.maskIcon,{color:e.themeColor||"#46bd87"})):e.apple!==!1&&i&&(s("apple-touch-icon",i),r("apple-mobile-web-app-capable","yes"),r("apple-mobile-web-app-status-bar-style","black")),typeof e.msTile=="object"&&(e.msTile.image||i)?(r("msapplication-TileImage",e.msTile.image||i),r("msapplication-TileColor",e.msTile.color||e.themeColor||"#46bd87")):e.msTile!==!1&&i&&(r("msapplication-TileImage",i),r("msapplication-TileColor",e.themeColor||"#46bd87")),r("viewport","width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover"),n},H=C(import.meta.url),f=P(b.resolve(H,"../client")),M=(e,a)=>{let n="",t="";return a.showInstall&&(n+=`import PWAInstall from "${f}components/PWAInstall.js";
`,t+=`PWAInstall,
`),a.update==="hint"?(n+=`import SWHintPopup from "${a.hintComponent||`${f}components/SWHintPopup.js`}";
`,t+=`SWHintPopup,
`):a.update!=="disable"&&a.update!=="force"&&(n+=`import SWUpdatePopup from "${a.updateComponent||`${f}components/SWUpdatePopup.js`}";
`,t+=`SWUpdatePopup,
`),e.writeTemp("pwa2/config.js",`import { defineClientConfig } from "@vuepress/client";
import { setupPWA } from "${f}composables/setup.js";
${n}
...
`)},L=(e={},a=!1)=>n=>{a&&T(e),n.env.isDebug&&m.info("Options:",e);const{base:t,shouldPrefetch:o=!0}=n.options;e.appendBase&&D(t,e),o===!0&&m.warn('The plugin will register service worker to handle assets, so we recommend you to set "shouldPrefetch: false" in VuePress config file.');const l=O(n,e);return n.options.head=B(e,t,n.options.head),k(n,{id:"hope"}),{name:"vuepress-plugin-pwa2",define:()=>({PWA_LOCALES:z({app:n,name:"pwa",default:v,config:e.locales}),SW_FORCE_UPDATE:e.update==="force",SW_PATH:e.swPath||"service-worker.js"}),extendsBundlerOptions:(i,s)=>{S({app:s,config:i},["mitt","register-service-worker"]),x({app:s,config:i},{path:"/manifest.webmanifest",response:async()=>JSON.stringify(await l),errMsg:"Unexpected manifest generate error"})},onGenerated:async i=>{await $(i,l),await W(i,e)},clientConfigFile:i=>M(i,e)}};export{v as pwaLocales,L as pwaPlugin};
