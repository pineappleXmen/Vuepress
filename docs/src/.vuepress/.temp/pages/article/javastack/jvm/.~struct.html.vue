<template><div><figure><img src="/javastack/jvm/image-20220823220237424.png" alt="image-20220823220237424" tabindex="0" loading="lazy"><figcaption>image-20220823220237424</figcaption></figure>
<blockquote>
...
<h2 id="_1-程序计数器-program-counter-register" tabindex="-1"><a class="header-anchor" href="#_1-程序计数器-program-counter-register" aria-hidden="true">#</a> 1.程序计数器 Program Counter Register</h2>
<p>JVM的程序计数器与CPU的程序计数器不同，但作用都有相似的地方。</p>
<p>首先看看CPU程序计数器的概念：</p>
<blockquote>
...
<p>而JVM中的程序计数器是在软件层面上的计数器</p>
<p>其作用，是<strong>记住下一条字节码的jvm指令的执行地址</strong></p>
<h3 id="特点" tabindex="-1"><a class="header-anchor" href="#特点" aria-hidden="true">#</a> <strong>特点</strong></h3>
<ul>
<li>是线程私有的</li>
<li>不会存在内存溢出</li>
<li>在执行java方法时，程序计数器是有值的，执行native本地方法时，程序计数器的值为空。</li>
</ul>
<p>任何时间一个线程都只有一个方法在执行，也就是所谓的当前方法。程序计数器会存储当前线程正在执行的java方法的JVM指令地址；或者，如果正在执行native方法，则是未指定值（undefined）。</p>
<p>因为native方法是java通过JNI直接调用本地C/C++库，可以近似的认为native方法
相当于C/C++暴露给java的一个接口，java通过调用这个接口从而调用到C/C++方法。
由于该方法是通过C/C++而不是java进行实现。那么自然无法产生相应的字节码，并
且C/C++执行时的内存分配是由自己语言决定的，而不是由JVM决定的。</p>
<h3 id="作用" tabindex="-1"><a class="header-anchor" href="#作用" aria-hidden="true">#</a> <strong>作用</strong></h3>
...
26: invokevirtual #26                 // --
29: return
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>左侧的数字即为字节码指令的编号</p>
<h2 id="_2-虚拟机栈" tabindex="-1"><a class="header-anchor" href="#_2-虚拟机栈" aria-hidden="true">#</a> 2.虚拟机栈</h2>
...
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/jvm/stack-area.png" alt="Java 虚拟机栈" tabindex="0" loading="lazy"><figcaption>Java 虚拟机栈</figcaption></figure>
...
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/jvm/local-variables-table.png" alt="局部变量表" tabindex="0" loading="lazy"><figcaption>局部变量表</figcaption></figure>
...
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/jvmimage-20220331175738692.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>栈空间虽然不是无限的，但一般正常调用的情况下是不会出现问题的。不过，如果函数调用陷入无限循环的话，就会导致栈中被压入太多栈帧而占用太多空间，导致栈空间过深。那么当线程请求栈的深度超过当前 Java 虚拟机栈的最大深度的时候，就抛出 <code v-pre>StackOverFlowError</code> 错误。</p>
...
<p>除了 <code v-pre>StackOverFlowError</code> 错误之外，栈还可能会出现<code v-pre>OutOfMemoryError</code>错误，这是因为如果栈的内存大小可以动态扩展， 如果虚拟机在动态扩展栈时无法申请到足够的内存空间，则抛出<code v-pre>OutOfMemoryError</code>异常。</p>
<p>简单总结一下程序运行中栈可能会出现两种错误：</p>
<ul>
<li><strong><code v-pre>StackOverFlowError</code>：</strong> 若栈的内存大小不允许动态扩展，那么当线程请求栈的深度超过当前 Java 虚拟机栈的最大深度的时候，就抛出 <code v-pre>StackOverFlowError</code> 错误。</li>
<li><strong><code v-pre>OutOfMemoryError</code>：</strong> 如果栈的内存大小可以动态扩展， 如果虚拟机在动态扩展栈时无法申请到足够的内存空间，则抛出<code v-pre>OutOfMemoryError</code>异常。</li>
</ul>
<h3 id="线程运行诊断" tabindex="-1"><a class="header-anchor" href="#线程运行诊断" aria-hidden="true">#</a> 线程运行诊断</h3>
<p>案例1： cpu 占用过多
定位
用top定位哪个进程对cpu的占用过高
ps H -eo pid,tid,%cpu | grep 进程id （用ps命令进一步定位是哪个线程引起的cpu占用过高）
jstack 进程id
可以根据线程id 找到有问题的线程，进一步定位到问题代码的源码行号</p>
...
<figure><img src="https://javaguide.cn/assets/hotspot-heap-structure.41533631.png" alt="hotspot-heap-structure" tabindex="0" loading="lazy"><figcaption>hotspot-heap-structure</figcaption></figure>
...
<h3 id="方法区内存溢出" tabindex="-1"><a class="header-anchor" href="#方法区内存溢出" aria-hidden="true">#</a> 方法区内存溢出</h3>
<p>1.8 以前会导致永久代内存溢出
1.8 之后会导致元空间内存溢出</p>
<p>场景
mybatis</p>
...
<h3 id="运行时常量池" tabindex="-1"><a class="header-anchor" href="#运行时常量池" aria-hidden="true">#</a> 运行时常量池</h3>
...
<p>既然运行时常量池是方法区的一部分，自然受到方法区内存的限制，当常量池无法再申请到内存时会抛出 <code v-pre>OutOfMemoryError</code> 错误。</p>
<h3 id="字符串常量池" tabindex="-1"><a class="header-anchor" href="#字符串常量池" aria-hidden="true">#</a> 字符串常量池</h3>
<p><strong>字符串常量池</strong> 是 JVM 为了提升性能和减少内存消耗针对字符串（String 类）专门开辟的一块区域，主要目的是为了避免字符串的重复创建。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 在堆中创建字符串对象”ab“</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>HotSpot 虚拟机中字符串常量池的实现是 <code v-pre>src/hotspot/share/classfile/stringTable.cpp</code> ,<code v-pre>StringTable</code> 本质上就是一个<code v-pre>HashSet&lt;String&gt;</code> ,容量为 <code v-pre>StringTableSize</code>（可以通过 <code v-pre>-XX:StringTableSize</code> 参数来设置）。</p>
<p><strong><code v-pre>StringTable</code> 中保存的是字符串对象的引用，字符串对象的引用指向堆中的字符串对象。</strong></p>
<p>JDK1.7 之前，字符串常量池存放在永久代。JDK1.7 字符串常量池和静态变量从永久代移动了 Java 堆中。</p>
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/jvm/method-area-jdk1.6.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/jvm/method-area-jdk1.7.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/jvm/method-area-jdk1.8.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
...
<p>相关问题：<a href="https://www.zhihu.com/question/57109429/answer/151717241" target="_blank" rel="noopener noreferrer">JVM 常量池中存储的是对象还是引用呢？ - RednaxelaFX - 知乎open in new window<ExternalLinkIcon/></a></p>
<p>最后再来分享一段周志明老师在<a href="https://github.com/fenixsoft/jvm_book" target="_blank" rel="noopener noreferrer">《深入理解 Java 虚拟机（第 3 版）》样例代码&amp;勘误open in new window<ExternalLinkIcon/></a> Github 仓库的 <a href="https://github.com/fenixsoft/jvm_book/issues/112" target="_blank" rel="noopener noreferrer">issue#112open in new window<ExternalLinkIcon/></a> 中说过的话：</p>
<blockquote>
<p><strong>运行时常量池、方法区、字符串常量池这些都是不随虚拟机实现而改变的逻辑概念，是公共且抽象的，Metaspace、Heap 是与具体某种虚拟机实现相关的物理概念，是私有且具体的。</strong></p>
</blockquote>
<h3 id="stringtable" tabindex="-1"><a class="header-anchor" href="#stringtable" aria-hidden="true">#</a> StringTable</h3>
<p>常量池中的字符串仅是符号，第一次用到时才变为对象
...
分配回收成本较高，但读写性能高
不受 JVM 内存回收管理</p>
<h3 id="分配和回收原理" tabindex="-1"><a class="header-anchor" href="#分配和回收原理" aria-hidden="true">#</a> 分配和回收原理</h3>
