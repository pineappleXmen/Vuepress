<template><div><h2 id="_1-java基础概念" tabindex="-1"><a class="header-anchor" href="#_1-java基础概念" aria-hidden="true">#</a> 1.Java基础概念</h2>
<h3 id="_1-java-语言有哪些特点" tabindex="-1"><a class="header-anchor" href="#_1-java-语言有哪些特点" aria-hidden="true">#</a> 1.Java 语言有哪些特点?</h3>
<ol>
<li>简单易学；</li>
<li>面向对象（封装，继承，多态）；</li>
...
<blockquote>
<p><strong>🐛 修正（参见： <a href="https://github.com/Snailclimb/JavaGuide/issues/544" target="_blank" rel="noopener noreferrer">issue#544open in new window<ExternalLinkIcon/></a>）</strong> ：C++11 开始（2011 年的时候）,C++就引入了多线程库，在 windows、linux、macos 都可以使用<code v-pre>std::thread</code>和<code v-pre>std::async</code>来创建线程。参考链接：<a href="http://www.cplusplus.com/reference/thread/thread/?kw=thread" target="_blank" rel="noopener noreferrer">http://www.cplusplus.com/reference/thread/thread/?kw=thread<ExternalLinkIcon/></a></p>
</blockquote>
<p>🌈 拓展一下：</p>
...
<h3 id="_2-jvm-vs-jdk-vs-jre" tabindex="-1"><a class="header-anchor" href="#_2-jvm-vs-jdk-vs-jre" aria-hidden="true">#</a> 2.JVM vs JDK vs JRE</h3>
<h4 id="jvm" tabindex="-1"><a class="header-anchor" href="#jvm" aria-hidden="true">#</a> JVM</h4>
...
<p>除了我们平时最常用的 HotSpot VM 外，还有 J9 VM、Zing VM、JRockit VM 等 JVM 。维基百科上就有常见 JVM 的对比：<a href="https://en.wikipedia.org/wiki/Comparison_of_Java_virtual_machines" target="_blank" rel="noopener noreferrer">Comparison of Java virtual machinesopen in new window<ExternalLinkIcon/></a> ，感兴趣的可以去看看。并且，你可以在 <a href="https://docs.oracle.com/javase/specs/index.html" target="_blank" rel="noopener noreferrer">Java SE Specificationsopen in new window<ExternalLinkIcon/></a> 上找到各个版本的 JDK 对应的 JVM 规范。</p>
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/basis/JavaSeSpecifications.jpg" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<h4 id="jdk-和-jre" tabindex="-1"><a class="header-anchor" href="#jdk-和-jre" aria-hidden="true">#</a> JDK 和 JRE</h4>
...
<h3 id="_3-什么是字节码-采用字节码的好处是什么" tabindex="-1"><a class="header-anchor" href="#_3-什么是字节码-采用字节码的好处是什么" aria-hidden="true">#</a> 3.什么是字节码?采用字节码的好处是什么?</h3>
<p>在 Java 中，JVM 可以理解的代码就叫做字节码（即扩展名为 <code v-pre>.class</code> 的文件），它不面向任何特定的处理器，只面向虚拟机。Java 语言通过字节码的方式，在一定程度上解决了传统解释型语言执行效率低的问题，同时又保留了解释型语言可移植的特点。所以， Java 程序运行时相对来说还是高效的（不过，和 C++，Rust，Go 等语言还是有一定差距的），而且，由于字节码并不针对一种特定的机器，因此，Java 程序无须重新编译便可在多种不同操作系统的计算机上运行。</p>
<p><strong>Java 程序从源代码到运行的过程如下图所示：</strong></p>
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/basis/java-code-to-machine-code.png" alt="Java程序转变为机器代码的过程" tabindex="0" loading="lazy"><figcaption>Java程序转变为机器代码的过程</figcaption></figure>
<p>我们需要格外注意的是 <code v-pre>.class-&gt;机器码</code> 这一步。在这一步 JVM 类加载器首先加载字节码文件，然后通过解释器逐行解释执行，这种方式的执行速度会相对比较慢。而且，有些方法和代码块是经常需要被调用的(也就是所谓的热点代码)，所以后面引进了 JIT（just-in-time compilation） 编译器，而 JIT 属于运行时编译。当 JIT 编译器完成第一次编译后，其会将字节码对应的机器码保存下来，下次可以直接使用。而我们知道，机器码的运行效率肯定是高于 Java 解释器的。这也解释了我们为什么经常会说 <strong>Java 是编译与解释共存的语言</strong> 。</p>
<blockquote>
...
<h3 id="_4-为什么不全部使用-aot-呢" tabindex="-1"><a class="header-anchor" href="#_4-为什么不全部使用-aot-呢" aria-hidden="true">#</a> 4.为什么不全部使用 AOT 呢？</h3>
<p>AOT 可以提前编译节省启动时间，那为什么不全部使用这种编译方式呢？</p>
<p>长话短说，这和 Java 语言的动态特性有千丝万缕的联系了。举个例子，CGLIB 动态代理使用的是 ASM 技术，而这种技术大致原理是运行时直接在内存中生成并加载修改后的字节码文件也就是 <code v-pre>.class</code> 文件，如果全部使用 AOT 提前编译，也就不能使用 ASM 技术了。为了支持类似的动态特性，所以选择使用 JIT 即时编译器。</p>
<h3 id="_5-为什么说-java-语言-编译与解释并存" tabindex="-1"><a class="header-anchor" href="#_5-为什么说-java-语言-编译与解释并存" aria-hidden="true">#</a> 5.为什么说 Java 语言“编译与解释并存”？</h3>
<p>其实这个问题我们讲字节码的时候已经提到过，因为比较重要，所以我们这里再提一下。</p>
<p>我们可以将高级编程语言按照程序的执行方式分为两种：</p>
<ul>
<li><strong>编译型</strong> ：<a href="https://zh.wikipedia.org/wiki/%E7%B7%A8%E8%AD%AF%E8%AA%9E%E8%A8%80" target="_blank" rel="noopener noreferrer">编译型语言open in new window<ExternalLinkIcon/></a> 会通过<a href="https://zh.wikipedia.org/wiki/%E7%B7%A8%E8%AD%AF%E5%99%A8" target="_blank" rel="noopener noreferrer">编译器open in new window<ExternalLinkIcon/></a>将源代码一次性翻译成可被该平台执行的机器码。一般情况下，编译语言的执行速度比较快，开发效率比较低。常见的编译性语言有 C、C++、Go、Rust 等等。</li>
<li><strong>解释型</strong> ：<a href="https://zh.wikipedia.org/wiki/%E7%9B%B4%E8%AD%AF%E8%AA%9E%E8%A8%80" target="_blank" rel="noopener noreferrer">解释型语言open in new window<ExternalLinkIcon/></a>会通过<a href="https://zh.wikipedia.org/wiki/%E7%9B%B4%E8%AD%AF%E5%99%A8" target="_blank" rel="noopener noreferrer">解释器open in new window<ExternalLinkIcon/></a>一句一句的将代码解释（interpret）为机器代码后再执行。解释型语言开发效率比较快，执行速度比较慢。常见的解释性语言有 Python、JavaScript、PHP 等等。</li>
</ul>
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/basis/compiled-and-interpreted-languages.png" alt="编译型语言和解释型语言" tabindex="0" loading="lazy"><figcaption>编译型语言和解释型语言</figcaption></figure>
<p>根据维基百科介绍：</p>
<blockquote>
<p>为了改善编译语言的效率而发展出的<a href="https://zh.wikipedia.org/wiki/%E5%8D%B3%E6%99%82%E7%B7%A8%E8%AD%AF" target="_blank" rel="noopener noreferrer">即时编译open in new window<ExternalLinkIcon/></a>技术，已经缩小了这两种语言间的差距。这种技术混合了编译语言与解释型语言的优点，它像编译语言一样，先把程序源代码编译成<a href="https://zh.wikipedia.org/wiki/%E5%AD%97%E8%8A%82%E7%A0%81" target="_blank" rel="noopener noreferrer">字节码open in new window<ExternalLinkIcon/></a>。到执行期时，再将字节码直译，之后执行。<a href="https://zh.wikipedia.org/wiki/Java" target="_blank" rel="noopener noreferrer">Javaopen in new window<ExternalLinkIcon/></a>与<a href="https://zh.wikipedia.org/wiki/LLVM" target="_blank" rel="noopener noreferrer">LLVMopen in new window<ExternalLinkIcon/></a>是这种技术的代表产物。</p>
<p>相关阅读：<a href="https://tech.meituan.com/2020/10/22/java-jit-practice-in-meituan.html" target="_blank" rel="noopener noreferrer">基本功 | Java 即时编译器原理解析及实践open in new window<ExternalLinkIcon/></a></p>
</blockquote>
<p><strong>为什么说 Java 语言“编译与解释并存”？</strong></p>
<p>这是因为 Java 语言既具有编译型语言的特征，也具有解释型语言的特征。因为 Java 程序要经过先编译，后解释两个步骤，由 Java 编写的程序需要先经过编译步骤，生成字节码（<code v-pre>.class</code> 文件），这种字节码必须由 Java 解释器来解释执行。</p>
<h3 id="_6-oracle-jdk-vs-openjdk" tabindex="-1"><a class="header-anchor" href="#_6-oracle-jdk-vs-openjdk" aria-hidden="true">#</a> 6.Oracle JDK vs OpenJDK</h3>
...
<p>答：</p>
<ol>
<li>OpenJDK 是开源的，开源意味着你可以对它根据你自己的需要进行修改、优化，比如 Alibaba 基于 OpenJDK 开发了 Dragonwell8：<a href="https://github.com/alibaba/dragonwell8" target="_blank" rel="noopener noreferrer">https://github.com/alibaba/dragonwell8open in new window<ExternalLinkIcon/></a></li>
...
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/basis/oracle-jdk-release-cadence.jpg" alt="oracle jdk release cadence" tabindex="0" loading="lazy"><figcaption>oracle jdk release cadence</figcaption></figure>
<p>🌈 拓展一下：</p>
<ul>
...
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/java-guide-blog/20210425151835918.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>相关阅读 👍：<a href="https://www.baeldung.com/oracle-jdk-vs-openjdk" target="_blank" rel="noopener noreferrer">《Differences Between Oracle JDK and OpenJDK》open in new window<ExternalLinkIcon/></a></p>
<h3 id="_7-java-和-c-的区别" tabindex="-1"><a class="header-anchor" href="#_7-java-和-c-的区别" aria-hidden="true">#</a> 7.Java 和 C++ 的区别?</h3>
...
<h2 id="_2-基本语法" tabindex="-1"><a class="header-anchor" href="#_2-基本语法" aria-hidden="true">#</a> 2.基本语法</h2>
<h3 id="_1-注释有哪几种形式" tabindex="-1"><a class="header-anchor" href="#_1-注释有哪几种形式" aria-hidden="true">#</a> 1.注释有哪几种形式？</h3>
<p>Java 中的注释有三种：</p>
<ol>
...
<figure><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/github/javaguide/java/basis/image-20220714112336911.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// check to see if the employee is eligible for full benefits</span>
...
<h3 id="标识符和关键字的区别是什么" tabindex="-1"><a class="header-anchor" href="#标识符和关键字的区别是什么" aria-hidden="true">#</a> 标识符和关键字的区别是什么？</h3>
...
<h3 id="java-语言关键字有哪些" tabindex="-1"><a class="header-anchor" href="#java-语言关键字有哪些" aria-hidden="true">#</a> Java 语言关键字有哪些？</h3>
<table>
<thead>
<tr>
...
<p><code v-pre>default</code> 这个关键字很特殊，既属于程序控制，也属于类，方法和变量修饰符，还属于访问控制。</p>
<ul>
<li>在程序控制中，当在 <code v-pre>switch</code> 中匹配不到任何情况时，可以使用 <code v-pre>default</code> 来编写默认匹配的情况。</li>
<li>在类，方法和变量修饰符中，从 JDK8 开始引入了默认方法，可以使用 <code v-pre>default</code> 关键字来定义一个方法的默认实现。</li>
<li>在访问控制中，如果一个方法前没有任何修饰符，则默认会有一个修饰符 <code v-pre>default</code>，但是这个修饰符加上了就会报错。</li>
</ul>
</blockquote>
<p>⚠️ 注意 ：虽然 <code v-pre>true</code>, <code v-pre>false</code>, 和 <code v-pre>null</code> 看起来像关键字但实际上他们是字面值，同时你也不可以作为标识符来使用。</p>
<p>官方文档：<a href="https://docs.oracle.com/javase/tutorial/java/nutsandbolts/_keywords.html" target="_blank" rel="noopener noreferrer">https://docs.oracle.com/javase/tutorial/java/nutsandbolts/_keywords.htmlopen in new window<ExternalLinkIcon/></a></p>
<h3 id="自增自减运算符" tabindex="-1"><a class="header-anchor" href="#自增自减运算符" aria-hidden="true">#</a> 自增自减运算符</h3>
<p>在写代码的过程中，常见的一种情况是需要某个整数类型变量增加 1 或减少 1，Java 提供了一种特殊的运算符，用于这种表达式，叫做自增运算符（++)和自减运算符（--）。</p>
<p>++ 和 -- 运算符可以放在变量之前，也可以放在变量之后，当运算符放在变量之前时(前缀)，先自增/减，再赋值；当运算符放在变量之后时(后缀)，先赋值，再自增/减。例如，当 <code v-pre>b = ++a</code> 时，先自增（自己增加 1），再赋值（赋值给 b）；当 <code v-pre>b = a++</code> 时，先赋值(赋值给 b)，再自增（自己增加 1）。也就是，++a 输出的是 a+1 的值，a++输出的是 a 值。用一句口诀就是：“符号在前就先加/减，符号在后就后加/减”。</p>
<h3 id="continue、break-和-return-的区别是什么" tabindex="-1"><a class="header-anchor" href="#continue、break-和-return-的区别是什么" aria-hidden="true">#</a> continue、break 和 return 的区别是什么？</h3>
<p>在循环结构中，当循环条件不满足或者循环次数达到要求时，循环会正常结束。但是，有时候可能需要在循环的过程中，当发生了某种条件之后 ，提前终止循环，这就需要用到下面几个关键词：</p>
<ol>
<li><code v-pre>continue</code> ：指跳出当前的这一次循环，继续下一次循环。</li>
<li><code v-pre>break</code> ：指跳出整个循环体，继续执行循环下面的语句。</li>
</ol>
<p><code v-pre>return</code> 用于跳出所在方法，结束该方法的运行。return 一般有两种用法：</p>
<ol>
<li><code v-pre>return;</code> ：直接使用 return 结束方法执行，用于没有返回值函数的方法</li>
<li><code v-pre>return value;</code> ：return 一个特定值，用于有返回值函数的方法</li>
</ol>
...
25</p>
<p>运行结果：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>0
xixi
1
2
xixi
3
haha
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>1
2
3
4
5
...
<h4 id="成员变量与局部变量的区别" tabindex="-1"><a class="header-anchor" href="#成员变量与局部变量的区别" aria-hidden="true">#</a> 成员变量与局部变量的区别？</h4>
<ul>
<li><strong>语法形式</strong> ：从语法形式上看，成员变量是属于类的，而局部变量是在代码块或方法中定义的变量或是方法的参数；成员变量可以被 <code v-pre>public</code>,<code v-pre>private</code>,<code v-pre>static</code> 等修饰符所修饰，而局部变量不能被访问控制修饰符及 <code v-pre>static</code> 所修饰；但是，成员变量和局部变量都能被 <code v-pre>final</code> 所修饰。</li>
<li><strong>存储方式</strong> ：从变量在内存中的存储方式来看,如果成员变量是使用 <code v-pre>static</code> 修饰的，那么这个成员变量是属于类的，如果没有使用 <code v-pre>static</code> 修饰，这个成员变量是属于实例的。而对象存在于堆内存，局部变量则存在于栈内存。</li>
<li><strong>生存时间</strong> ：从变量在内存中的生存时间上看，成员变量是对象的一部分，它随着对象的创建而存在，而局部变量随着方法的调用而自动生成，随着方法的调用结束而消亡。</li>
<li><strong>默认值</strong> ：从变量是否有默认值来看，成员变量如果没有被赋初始值，则会自动以类型的默认值而赋值（一种情况例外:被 <code v-pre>final</code> 修饰的成员变量也必须显式地赋值），而局部变量则不会自动赋值。</li>
</ul>
<h4 id="静态变量有什么作用" tabindex="-1"><a class="header-anchor" href="#静态变量有什么作用" aria-hidden="true">#</a> 静态变量有什么作用？</h4>
<p>静态变量可以被类的所有实例共享。无论一个类创建了多少个对象，它们都共享同一份静态变量。</p>
<p>通常情况下，静态变量会被 <code v-pre>final</code> 关键字修饰成为常量。</p>
<h4 id="字符型常量和字符串常量的区别" tabindex="-1"><a class="header-anchor" href="#字符型常量和字符串常量的区别" aria-hidden="true">#</a> 字符型常量和字符串常量的区别?</h4>
<ol>
...
<p>(<strong>注意： <code v-pre>char</code> 在 Java 中占两个字节</strong>)</p>
<h3 id="方法" tabindex="-1"><a class="header-anchor" href="#方法" aria-hidden="true">#</a> 方法</h3>
<h4 id="什么是方法的返回值-方法有哪几种类型" tabindex="-1"><a class="header-anchor" href="#什么是方法的返回值-方法有哪几种类型" aria-hidden="true">#</a> 什么是方法的返回值?方法有哪几种类型？</h4>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">void</span> <span class="token function">f1</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">void</span> <span class="token function">f2</span><span class="token punctuation">(</span><span class="token class-name">Parameter</span> <span class="token number">1</span><span class="token punctuation">,</span> <span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">,</span> <span class="token class-name">Parameter</span> n<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">//......</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>1
2
3</p>
<p><strong>3.有返回值无参数的方法</strong></p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">int</span> <span class="token function">f3</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">//......</span>
    <span class="token keyword">return</span> x<span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>1
2
3
4</p>
...
2
3</p>
<h4 id="静态方法为什么不能调用非静态成员" tabindex="-1"><a class="header-anchor" href="#静态方法为什么不能调用非静态成员" aria-hidden="true">#</a> 静态方法为什么不能调用非静态成员?</h4>
...
<h4 id="静态方法和实例方法有何不同" tabindex="-1"><a class="header-anchor" href="#静态方法和实例方法有何不同" aria-hidden="true">#</a> 静态方法和实例方法有何不同？</h4>
<p><strong>1、调用方式</strong></p>
<p>在外部调用静态方法时，可以使用 <code v-pre>类名.方法名</code> 的方式，也可以使用 <code v-pre>对象.方法名</code> 的方式，而实例方法只有后面这种方式。也就是说，<strong>调用静态方法可以无需创建对象</strong> 。</p>
<p>不过，需要注意的是一般不建议使用 <code v-pre>对象.方法名</code> 的方式来调用静态方法。这种方式非常容易造成混淆，静态方法不属于类的某个对象而是属于这个类。</p>
<p>因此，一般建议使用 <code v-pre>类名.方法名</code> 的方式来调用静态方法。</p>
...
<h4 id="重载和重写有什么区别" tabindex="-1"><a class="header-anchor" href="#重载和重写有什么区别" aria-hidden="true">#</a> 重载和重写有什么区别？</h4>
<blockquote>
...
<blockquote>
<p>如果多个方法(比如 <code v-pre>StringBuilder</code> 的构造方法)有相同的名字、不同的参数， 便产生了重载。</p>
...
<li>如果父类方法访问修饰符为 <code v-pre>private/final/static</code> 则子类就不能重写该方法，但是被 <code v-pre>static</code> 修饰的方法能够被再次声明。</li>
<li>构造方法无法被重写</li>
</ol>
...
<p><strong>方法的重写要遵循“两同两小一大”</strong>（以下内容摘录自《疯狂 Java 讲义》，<a href="https://github.com/Snailclimb/JavaGuide/issues/892" target="_blank" rel="noopener noreferrer">issue#892open in new window<ExternalLinkIcon/></a> ）：</p>
<ul>
...
24
25</p>
<h4 id="什么是可变长参数" tabindex="-1"><a class="header-anchor" href="#什么是可变长参数" aria-hidden="true">#</a> 什么是可变长参数？</h4>
<p>从 Java5 开始，Java 支持定义可变长参数，所谓可变长参数就是允许在调用方法时传入不定长度的参数。就比如下面的这个 <code v-pre>printVariable</code> 方法就可以接受 0 个或者多个参数。</p>
...
23</p>
<p>输出：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>ab
a
b
c
d
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>1
2
3
4
5</p>
<p>另外，Java 的可变参数编译后实际会被转换成一个数组，我们看编译后生成的 <code v-pre>class</code>文件就可以看出来了。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">VariableLengthArgument</span> <span class="token punctuation">{</span>
...
13
14</p>
<h2 id="_3-基本数据类型" tabindex="-1"><a class="header-anchor" href="#_3-基本数据类型" aria-hidden="true">#</a> 3.基本数据类型</h2>
<h3 id="_1-java-中的几种基本数据类型了解么" tabindex="-1"><a class="header-anchor" href="#_1-java-中的几种基本数据类型了解么" aria-hidden="true">#</a> 1.Java 中的几种基本数据类型了解么？</h3>
...
<ol>
<li>Java 里使用 <code v-pre>long</code> 类型的数据一定要在数值后面加上 <strong>L</strong>，否则将作为整型解析。</li>
<li><code v-pre>char a = 'h'</code>char :单引号，<code v-pre>String a = &quot;hello&quot;</code> :双引号。</li>
</ol>
<p>这八种基本类型都有对应的包装类分别为：<code v-pre>Byte</code>、<code v-pre>Short</code>、<code v-pre>Integer</code>、<code v-pre>Long</code>、<code v-pre>Float</code>、<code v-pre>Double</code>、<code v-pre>Character</code>、<code v-pre>Boolean</code> 。</p>
<h3 id="_2-基本类型和包装类型的区别" tabindex="-1"><a class="header-anchor" href="#_2-基本类型和包装类型的区别" aria-hidden="true">#</a> 2.基本类型和包装类型的区别？</h3>
<ul>
...
<p>⚠️ 注意 ： <strong>基本数据类型存放在栈中是一个常见的误区！</strong> 基本数据类型的成员变量如果没有被 <code v-pre>static</code> 修饰的话（不建议这么使用，应该要使用基本数据类型对应的包装类型），就存放在堆中。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">BasicTypeVar</span><span class="token punctuation">{</span>
  <span class="token keyword">private</span> <span class="token keyword">int</span> x<span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>1
2
3</p>
<h3 id="_3-包装类型的缓存机制了解么" tabindex="-1"><a class="header-anchor" href="#_3-包装类型的缓存机制了解么" aria-hidden="true">#</a> 3.包装类型的缓存机制了解么？</h3>
...
12
13</p>
<p><strong><code v-pre>Character</code> 缓存源码:</strong></p>
...
15
16</p>
<p><strong><code v-pre>Boolean</code> 缓存源码：</strong></p>
...
<p>两种浮点数类型的包装类 <code v-pre>Float</code>,<code v-pre>Double</code> 并没有实现缓存机制。</p>
...
10
11</p>
<p>下面我们来看一下问题。下面的代码的输出结果是 <code v-pre>true</code> 还是 <code v-pre>false</code> 呢？</p>
...
2
3</p>
<p><code v-pre>Integer i1=40</code> 这一行代码会发生装箱，也就是说这行代码等价于 <code v-pre>Integer i1=Integer.valueOf(40)</code> 。因此，<code v-pre>i1</code> 直接使用的是缓存中的对象。而<code v-pre>Integer i2 = new Integer(40)</code> 会直接创建新的对象。</p>
<p>因此，答案是 <code v-pre>false</code> 。你答对了吗？</p>
<p>记住：<strong>所有整型包装类对象之间值的比较，全部使用 equals 方法比较</strong>。</p>
<figure><img src="https://img-blog.csdnimg.cn/20210422164544846.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<h3 id="_4-自动装箱与拆箱了解吗-原理是什么" tabindex="-1"><a class="header-anchor" href="#_4-自动装箱与拆箱了解吗-原理是什么" aria-hidden="true">#</a> 4.自动装箱与拆箱了解吗？原理是什么？</h3>
...
<p>举例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token class-name">Integer</span> i <span class="token operator">=</span> <span class="token number">10</span><span class="token punctuation">;</span>  <span class="token comment">//装箱</span>
...
2</p>
<p>上面这两行代码对应的字节码为：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>   <span class="token constant">L1</span>
...
<p>因此，</p>
<ul>
<li><code v-pre>Integer i = 10</code> 等价于 <code v-pre>Integer i = Integer.valueOf(10)</code></li>
<li><code v-pre>int n = i</code> 等价于 <code v-pre>int n = i.intValue()</code>;</li>
</ul>
...
6
7</p>
<h3 id="_5-为什么浮点数运算的时候会有精度丢失的风险" tabindex="-1"><a class="header-anchor" href="#_5-为什么浮点数运算的时候会有精度丢失的风险" aria-hidden="true">#</a> 5.为什么浮点数运算的时候会有精度丢失的风险？</h3>
<p>浮点数运算精度丢失代码演示：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">float</span> a <span class="token operator">=</span> <span class="token number">2.0f</span> <span class="token operator">-</span> <span class="token number">1.9f</span><span class="token punctuation">;</span>
...
<span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>1
2
3
4
5
...
7
8</p>
<p>关于浮点数的更多内容，建议看一下<a href="http://kaito-kidd.com/2018/08/08/computer-system-float-point/" target="_blank" rel="noopener noreferrer">计算机系统基础（四）浮点数open in new window<ExternalLinkIcon/></a>这篇文章。</p>
<h3 id="_6-如何解决浮点数运算的精度丢失问题" tabindex="-1"><a class="header-anchor" href="#_6-如何解决浮点数运算的精度丢失问题" aria-hidden="true">#</a> 6.如何解决浮点数运算的精度丢失问题？</h3>
...
9
10</p>
<p>关于 <code v-pre>BigDecimal</code> 的详细介绍，可以看看我写的这篇文章：<a href="https://javaguide.cn/java/basis/bigdecimal.html" target="_blank" rel="noopener noreferrer">BigDecimal 详解<ExternalLinkIcon/></a>。</p>
<h3 id="_7-超过-long-整型的数据应该如何表示" tabindex="-1"><a class="header-anchor" href="#_7-超过-long-整型的数据应该如何表示" aria-hidden="true">#</a> 7.超过 long 整型的数据应该如何表示？</h3>
...
2
3</p>
<p><code v-pre>BigInteger</code> 内部使用 <code v-pre>int[]</code> 数组来存储任意大小的整形数据。</p>
<p>相对于常规整数类型的运算来说，<code v-pre>BigInteger</code> 运算的效率会相对较低。</p>
</div></template>
