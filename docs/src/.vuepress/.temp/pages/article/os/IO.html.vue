<template><div><h2 id="_1-什么是dma技术" tabindex="-1"><a class="header-anchor" href="#_1-什么是dma技术" aria-hidden="true">#</a> 1.什么是DMA技术?</h2>
...
<p>为了方便你理解，我画了一副图：</p>
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/操作系统/零拷贝/I_O 中断.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
...
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/操作系统/零拷贝/DRM I_O 过程.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>具体过程：</p>
<ul>
...
<hr>
<h2 id="传统的文件传输有多糟糕" tabindex="-1"><a class="header-anchor" href="#传统的文件传输有多糟糕" aria-hidden="true">#</a> 传统的文件传输有多糟糕？</h2>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>代码很简单，虽然就两行代码，但是这里面发生了不少的事情。</p>
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/操作系统/零拷贝/传统文件传输.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>首先，期间共<strong>发生了 4 次用户态与内核态的上下文切换</strong>，因为发生了两次系统调用，一次是 <code v-pre>read()</code> ，一次是 <code v-pre>write()</code>，每次系统调用都得先从用户态切换到内核态，等内核完成任务后，再从内核态切换回用户态。</p>
...
<hr>
<h2 id="如何优化文件传输的性能" tabindex="-1"><a class="header-anchor" href="#如何优化文件传输的性能" aria-hidden="true">#</a> 如何优化文件传输的性能？</h2>
<blockquote>
...
<hr>
<h2 id="_2-如何实现零拷贝" tabindex="-1"><a class="header-anchor" href="#_2-如何实现零拷贝" aria-hidden="true">#</a> 2.如何实现零拷贝？</h2>
...
<h3 id="mmap-write" tabindex="-1"><a class="header-anchor" href="#mmap-write" aria-hidden="true">#</a> mmap + write</h3>
<p>在前面我们知道，<code v-pre>read()</code> 系统调用的过程中会把内核缓冲区的数据拷贝到用户的缓冲区里，于是为了减少这一步开销，我们可以用 <code v-pre>mmap()</code> 替换 <code v-pre>read()</code> 系统调用函数。</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code>buf <span class="token operator">=</span> <span class="token function">mmap</span><span class="token punctuation">(</span>file<span class="token punctuation">,</span> len<span class="token punctuation">)</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>mmap()</code> 系统调用函数会直接把内核缓冲区里的数据「<strong>映射</strong>」到用户空间，这样，操作系统内核与用户空间就不需要再进行任何的数据拷贝操作。</p>
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/操作系统/零拷贝/mmap %2B write 零拷贝.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>具体过程如下：</p>
<ul>
<li>应用进程调用了 <code v-pre>mmap()</code> 后，DMA 会把磁盘的数据拷贝到内核的缓冲区里。接着，应用进程跟操作系统内核「共享」这个缓冲区；</li>
<li>应用进程再调用 <code v-pre>write()</code>，操作系统直接将内核缓冲区的数据拷贝到 socket 缓冲区中，这一切都发生在内核态，由 CPU 来搬运数据；</li>
<li>最后，把内核的 socket 缓冲区里的数据，拷贝到网卡的缓冲区里，这个过程是由 DMA 搬运的。</li>
</ul>
<p>我们可以得知，通过使用 <code v-pre>mmap()</code> 来代替 <code v-pre>read()</code>， 可以减少一次数据拷贝的过程。</p>
<p>但这还不是最理想的零拷贝，因为仍然需要通过 CPU 把内核缓冲区的数据拷贝到 socket 缓冲区里，而且仍然需要 4 次上下文切换，因为系统调用还是 2 次。</p>
<h3 id="sendfile" tabindex="-1"><a class="header-anchor" href="#sendfile" aria-hidden="true">#</a> sendfile</h3>
<p>在 Linux 内核版本 2.1 中，提供了一个专门发送文件的系统调用函数 <code v-pre>sendfile()</code>，函数形式如下：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>它的前两个参数分别是目的端和源端的文件描述符，后面两个参数是源端的偏移量和复制数据的长度，返回值是实际复制数据的长度。</p>
<p>首先，它可以替代前面的 <code v-pre>read()</code> 和 <code v-pre>write()</code> 这两个系统调用，这样就可以减少一次系统调用，也就减少了 2 次上下文切换的开销。</p>
<p>其次，该系统调用，可以直接把内核缓冲区里的数据拷贝到 socket 缓冲区里，不再拷贝到用户态，这样就只有 2 次上下文切换，和 3 次数据拷贝。如下图：</p>
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/操作系统/零拷贝/senfile-3次拷贝.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">ethtool</span> <span class="token parameter variable">-k</span> eth0 <span class="token operator">|</span> <span class="token function">grep</span> scatter-gather
scatter-gather: on
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>于是，从 Linux 内核 <code v-pre>2.4</code> 版本开始起，对于支持网卡支持 SG-DMA 技术的情况下， <code v-pre>sendfile()</code> 系统调用的过程发生了点变化，具体过程如下：</p>
<ul>
...
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/操作系统/零拷贝/senfile-零拷贝.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
...
<h3 id="使用零拷贝技术的项目" tabindex="-1"><a class="header-anchor" href="#使用零拷贝技术的项目" aria-hidden="true">#</a> 使用零拷贝技术的项目</h3>
<p>事实上，Kafka 这个开源项目，就利用了「零拷贝」技术，从而大幅提升了 I/O 的吞吐率，这也是 Kafka 在处理海量数据为什么这么快的原因之一。</p>
<p>如果你追溯 Kafka 文件传输的代码，你会发现，最终它调用了 Java NIO 库里的 <code v-pre>transferTo</code> 方法：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Overridepublic</span> 
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果 Linux 系统支持 <code v-pre>sendfile()</code> 系统调用，那么 <code v-pre>transferTo()</code> 实际上最后就会使用到 <code v-pre>sendfile()</code> 系统调用函数。</p>
<p>曾经有大佬专门写过程序测试过，在同样的硬件条件下，传统文件传输和零拷拷贝文件传输的性能差异，你可以看到下面这张测试数据图，使用了零拷贝能够缩短 <code v-pre>65%</code> 的时间，大幅度提升了机器传输数据的吞吐量。</p>
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/操作系统/零拷贝/零拷贝测试数据.png" alt="数据来源于：https://developer.ibm.com/articles/j-zerocopy/" tabindex="0" loading="lazy"><figcaption>数据来源于：<a href="https://developer.ibm.com/articles/j-zerocopy/" target="_blank" rel="noopener noreferrer">https://developer.ibm.com/articles/j-zerocopy/<ExternalLinkIcon/></a></figcaption></figure>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>sendfile 配置的具体意思:</p>
<ul>
...
<hr>
<h2 id="_3-pagecache-有什么作用" tabindex="-1"><a class="header-anchor" href="#_3-pagecache-有什么作用" aria-hidden="true">#</a> 3.PageCache 有什么作用？</h2>
...
<p>比如，假设 read 方法每次只会读 <code v-pre>32 KB</code> 的字节，虽然 read 刚开始只会读 0 ～ 32 KB 的字节，但内核会把其后面的 32～64 KB 也读取到 PageCache，这样后面读取 32～64 KB 的成本就很低，如果在 32～64 KB 淘汰出 PageCache 前，进程读取到它了，收益就非常大。</p>
...
<hr>
<h2 id="_4-大文件传输用什么方式实现" tabindex="-1"><a class="header-anchor" href="#_4-大文件传输用什么方式实现" aria-hidden="true">#</a> 4.大文件传输用什么方式实现？</h2>
<p>那针对大文件的传输，我们应该使用什么方式呢？</p>
<p>我们先来看看最初的例子，当调用 read 方法读取文件时，进程实际上会阻塞在 read 方法调用，因为要等待磁盘数据的返回，如下图：</p>
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/操作系统/零拷贝/阻塞 IO 的过程.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>具体过程：</p>
<ul>
...
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/操作系统/零拷贝/异步 IO 的过程.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>它把读操作分为两部分：</p>
<ul>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>location /video/ { 
    sendfile on; 
    aio on; 
    directio 1024m; 
}
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>当文件大小大于 <code v-pre>directio</code> 值后，使用「异步 I/O + 直接 I/O」，否则使用「零拷贝技术」。</p>
<hr>
...
<p>为了提高文件传输的性能，于是就出现了零拷贝技术，它通过一次系统调用（<code v-pre>sendfile</code> 方法）合并了磁盘读取与网络发送两个操作，降低了上下文切换次数。另外，拷贝数据都是发生在内核中的，天然就降低了数据拷贝的次数。</p>
