<template><div><h2 id="_1-http是什么" tabindex="-1"><a class="header-anchor" href="#_1-http是什么" aria-hidden="true">#</a> 1.HTTP是什么</h2>
...
<h2 id="_2-http常见状态码" tabindex="-1"><a class="header-anchor" href="#_2-http常见状态码" aria-hidden="true">#</a> 2.HTTP常见状态码</h2>
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/6-五大类HTTP状态码.png" alt=" 五大类 HTTP 状态码 " tabindex="0" loading="lazy"><figcaption> 五大类 HTTP 状态码 </figcaption></figure>
<p><code v-pre>1xx</code> 类状态码属于<strong>提示信息</strong>，是协议处理中的一种中间状态，实际用到的比较少。</p>
<p><code v-pre>2xx</code> 类状态码表示服务器<strong>成功</strong>处理了客户端的请求，也是我们最愿意看到的状态。</p>
<ul>
<li>「<strong>200 OK</strong>」是最常见的成功状态码，表示一切正常。如果是非 <code v-pre>HEAD</code> 请求，服务器返回的响应头都会有 body 数据。</li>
<li>「<strong>204 No Content</strong>」无内容。服务器成功处理，但未返回内容。在未更新网页的情况下，可确保浏览器继续显示当前文档</li>
<li>「<strong>206 Partial Content</strong>」是应用于 HTTP 分块下载或断点续传，表示响应返回的 body 数据并不是资源的全部，而是其中的一部分，也是服务器处理成功的状态。</li>
</ul>
<p><code v-pre>3xx</code> 类状态码表示客户端请求的资源发生了变动，需要客户端用新的 URL 重新发送请求获取资源，也就是<strong>重定向</strong>。</p>
<ul>
<li>「<strong>300 Multiple Choices</strong>」多种选择。请求的资源可包括多个位置，相应可返回一个资源特征与地址的列表用于用户终端（例如：浏览器）选择</li>
...
<p>301 和 302 都会在响应头里使用字段 <code v-pre>Location</code>，指明后续要跳转的 URL，浏览器会自动重定向新的 URL。</p>
<ul>
<li>「<strong>304 Not Modified</strong>」未修改。所请求的资源未修改，服务器返回此状态码时，不会返回任何资源。客户端通常会缓存访问过的资源，通过提供一个头信息指出客户端希望只返回在指定日期之后修改的资源</li>
</ul>
<p><code v-pre>4xx</code> 类状态码表示客户端发送的<strong>报文有误</strong>，服务器无法处理，也就是错误码的含义。</p>
<ul>
...
<p><code v-pre>5xx</code> 类状态码表示客户端请求报文正确，但是<strong>服务器处理时内部发生了错误</strong>，属于服务器端的错误码。</p>
<ul>
...
<h2 id="_3-http常见字段" tabindex="-1"><a class="header-anchor" href="#_3-http常见字段" aria-hidden="true">#</a> 3.HTTP常见字段</h2>
<p><em><strong>Host字段</strong></em></p>
<p>指定服务器的域名</p>
<p><em><strong>Server</strong></em></p>
<p>服务器名称</p>
<p><em><strong>Content-Length 字段</strong></em></p>
<p>服务器在返回数据时，会有 <code v-pre>Content-Length</code> 字段，表明本次回应的数据长度。</p>
<p><em><strong>Content-Type 字段</strong></em></p>
<p><code v-pre>Content-Type</code> 字段用于服务器回应时，告诉客户端，本次数据是什么格式。</p>
<p><em><strong>Content-Encoding 字段</strong></em></p>
<p><code v-pre>Content-Encoding</code> 字段说明数据的压缩方法。表示服务器返回的数据使用了什么压缩格式</p>
<p>客户端在请求时，用 <code v-pre>Accept-Encoding</code> 字段说明自己可以接受哪些压缩方法。</p>
<p><em><strong>Location</strong></em></p>
<p>设置返回数据的另一个位置 301 302重定向时指定链接</p>
<p><em><strong>Accept-Charset</strong></em></p>
<p>设置接受的字符编码</p>
<p><em><strong>Accept-Datetime</strong></em></p>
<p>设置接受的版本时间</p>
<p><em><strong>Cookie</strong></em></p>
<p>设置服务器使用Set-Cookie发送的http cookie</p>
<p><em><strong>If-Modified-Since</strong></em></p>
<p>设置更新时间，从更新时间到服务端接受请求这段时间内如果资源没有改变，允许服务端返回304 Not Modified</p>
<p><em><strong>User-Agent</strong></em></p>
<p>用户代理的字符串值（用于服务器获取客户端信息 如操作系统等信息）</p>
<p><em><strong>Date</strong></em></p>
<p>设置消息发送的日期和时间</p>
<p><em><strong>ETag</strong></em></p>
<p>特定版本资源的标识符，通常是消息摘要</p>
<p><em><strong>Cache-Control</strong></em></p>
<p>设置请求响应链上所有的缓存机制必须遵守的指令</p>
<p><em><strong>Expires</strong></em></p>
<p>设置响应体的过期时间</p>
<p><em><strong>Last-Modified</strong></em></p>
<p>设置请求对象最后一次的修改日期</p>
<blockquote>
<p>Last-Modified: Tue, 15 Nov 1994 12:45:26 GMT</p>
</blockquote>
<p><em><strong>If-None-Match</strong></em></p>
<p>设置客户端ETag，如果和服务端接受请求生成的ETage相同，允许服务端返回304 Not Modified</p>
<blockquote>
<p>If-None-Match: “737060cd8c284d8af7ad3082f209582d”</p>
</blockquote>
<h2 id="_4-get-post" tabindex="-1"><a class="header-anchor" href="#_4-get-post" aria-hidden="true">#</a> 4.GET &amp; POST</h2>
...
<h3 id="get-和-post-方法都是安全和幂等的吗" tabindex="-1"><a class="header-anchor" href="#get-和-post-方法都是安全和幂等的吗" aria-hidden="true">#</a> GET 和 POST 方法都是安全和幂等的吗？</h3>
<p>先说明下安全和幂等的概念：</p>
<ul>
...
<h2 id="_5-http缓存" tabindex="-1"><a class="header-anchor" href="#_5-http缓存" aria-hidden="true">#</a> 5.HTTP缓存</h2>
<h3 id="强制缓存" tabindex="-1"><a class="header-anchor" href="#强制缓存" aria-hidden="true">#</a> <strong>强制缓存</strong></h3>
...
<ul>
<li>
<p><code v-pre>Cache-Control</code>， 是一个相对时间；</p>
<ul>
<li>
<table>
<thead>
<tr>
<th>Cache-Control常见字段</th>
<th>说明</th>
</tr>
</thead>
<tbody>
...
<tr>
<td>no-cache</td>
<td>必须先与服务器确认返回的响应是否被更改，然后才能使用该响应来满足后续对同一个网址的请求。因此，如果存在合适的验证令牌 (ETag)，no-cache 会发起往返通信来验证缓存的响应，如果资源未被更改，可以避免下载。</td>
</tr>
<tr>
<td>no-store</td>
...
<li>
<p><code v-pre>Expires</code>，Expires字段声明了一个网页或URL地址不再被浏览器缓存的时间，一旦超过了这个时间，浏览器都应该联系原始服务器。RFC告诉我们：“由于推断的失效时间也许会降低语义透明度，应该被谨慎使用，同时我们鼓励原始服务器尽可能提供确切的失效时间。” 　对于一般的纯静态页面，如html、gif、jpg、css、js，默认安装的Apache服务器，不会在响应头添加这个字段。Firefox浏览器接受到响应后，如果发现没有Expires字段，浏览器根据文件的类型和“<em><strong>Last-Modified</strong></em>”字段来推断出一个合适的失效时间，并存储在客户端。推测出的时间一般是接受到响应时间后的三天左右。</p>
</li>
</ul>
...
<p>当我们在浏览器使用开发者工具的时候，你可能会看到过某些请求的响应码是 <code v-pre>304</code>，这个是告诉浏览器可以使用本地缓存的资源，通常这种通过服务端告知客户端是否可以使用缓存的方式被称为协商缓存。</p>
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/网络/http1.1优化/缓存etag.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
...
<li>请求头部中的 <code v-pre>If-None-Match</code>：当资源过期时，浏览器发现响应头里有 Etag，则再次向服务器发起请求时，会将请求头If-None-Match 值设置为 Etag 的值。服务器收到请求后进行比对，如果资源没有变化返回 304，如果资源变化了返回 200。</li>
</ul>
...
<li>可能有些文件是在秒级以内修改的，<code v-pre>If-Modified-Since</code> 能检查到的粒度是秒级的，使用 Etag就能够保证这种需求下客户端在 1 秒内能刷新多次；</li>
...
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/network/http/http缓存.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
...
<h2 id="_6-http优缺点" tabindex="-1"><a class="header-anchor" href="#_6-http优缺点" aria-hidden="true">#</a> 6.HTTP优缺点</h2>
<h4 id="优点" tabindex="-1"><a class="header-anchor" href="#优点" aria-hidden="true">#</a> <strong>优点</strong></h4>
<p><em><strong>1. 简单</strong></em></p>
<p>HTTP 基本的报文格式就是 <code v-pre>header + body</code>，头部信息也是 <code v-pre>key-value</code> 简单文本的形式，<strong>易于理解</strong>，降低了学习和使用的门槛。</p>
<p><em><strong>2. 灵活和易于扩展</strong></em></p>
<p>HTTP协议里的各类请求方法、URI/URL、状态码、头字段等每个组成要求都没有被固定死，都允许开发人员<strong>自定义和扩充</strong>。</p>
<p>同时 HTTP 由于是工作在应用层（ <code v-pre>OSI</code> 第七层），则它<strong>下层可以随意变化</strong>。</p>
...
<h4 id="缺点" tabindex="-1"><a class="header-anchor" href="#缺点" aria-hidden="true">#</a> <strong>缺点</strong></h4>
<p><em><strong>1.无状态</strong></em></p>
<p><em><strong>2. 明文传输</strong></em></p>
<p><em><strong>3. 不安全</strong></em></p>
<p>长连接</p>
...
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/16-短连接与长连接.png" alt="短连接与长连接" tabindex="0" loading="lazy"><figcaption>短连接与长连接</figcaption></figure>
...
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/17-管道网络传输.png" alt="管道网络传输" tabindex="0" loading="lazy"><figcaption>管道网络传输</figcaption></figure>
...
<p>TIP</p>
<p>注意!</p>
<p>实际上 HTTP/1.1 管道化技术不是默认开启，而且浏览器基本都没有支持，所以<strong>后面所有文章讨论HTTP/1.1 都是建立在没有使用管道化的前提</strong>。大家知道有这个功能，但是没有被使用就行了。</p>
<p><em>3. 队头阻塞</em></p>
...
<figure><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/18-队头阻塞.png" alt="队头阻塞" tabindex="0" loading="lazy"><figcaption>队头阻塞</figcaption></figure>
<p>总之 HTTP/1.1 的性能一般般，后续的 HTTP/2 和 HTTP/3 就是在优化 HTTP 的性能。</p>
<h2 id="_7-rsa算法" tabindex="-1"><a class="header-anchor" href="#_7-rsa算法" aria-hidden="true">#</a> 7.RSA算法</h2>
<h2 id="_8-http2-0优化" tabindex="-1"><a class="header-anchor" href="#_8-http2-0优化" aria-hidden="true">#</a> 8.HTTP2.0优化</h2>
<p><em>1. 头部压缩</em></p>
<p>HTTP/2 会<strong>压缩头</strong>（Header）如果你同时发出多个请求，他们的头是一样的或是相似的，那么，协议会帮你<strong>消除重复的部分</strong>。</p>
<p>这就是所谓的 <code v-pre>HPACK</code> 算法：在客户端和服务器同时维护一张头信息表，所有字段都会存入这个表，生成一个索引号，以后就不发送同样字段了，只发送索引号，这样就<strong>提高速度</strong>了。</p>
<p><em>2. 二进制格式</em></p>
...
<p>比如下图，Stream 1 是客户端向服务端请求的资源，属于客户端建立的 Stream，所以该 Stream 的 ID 是奇数（数字1）；Stream 2 和 4 都是服务端主动向客户端推送的资源，属于服务端建立的 Stream，所以这两个 Stream 的 ID 是偶数（数字 2 和4）。</p>
<p>存在的问题</p>
