...
可以通过XOR运算获取半加和（S）的0 通过与运算获取进位（C）1，这样就实现了1位的加法计算器，这种计算器称为Half Adder(HA)，也就是半加法器。
...
如图共有16行*16列，16个数字可以用4位二进制（2^4=16）表示。如果要表示地址的行和列用8位数就可以进行表示了（如00000000 前4位表示第一行，后4位表示第一列）。
...
**寄存器方式**：操作数是处理器寄存器中的内容，直接给出寄存器名称即可。
